...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    min_diff_word = min(word_list,key = lambda w: diff_function(typed_word, w, limit))
...
def final_diff(start, goal, limit):
    ...
    return min(minimum_mewtations(start,goal,limit),sphinx_swaps(start,goal,limit))
...
def fastest_words(match):
    ...
    for word_index in word_indices:
        min_player_index = 0
        min_player_time = match["times"][min_player_index][word_index]
        for player_index in player_indices:
            if match["times"][player_index][word_index] < min_player_time:
                min_player_time = match["times"][player_index][word_index]
                min_player_index = player_index
        ret[min_player_index].append(word_at(match,word_index))        
    return ret
    # END PROBLEM 10
