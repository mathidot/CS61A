...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while steps_count < self.min_range and not place.is_hive:
            steps_count += 1
            place = place.entrance
        while steps_count <= self.max_range and not place.is_hive:
            if len(place.bees) > 0:
                break
            steps_count += 1
            place = place.entrance
        if steps_count > self.max_range or place.is_hive:
            return None
...
class TankAnt(ContainerAnt):
    ...
    """TankAnt provides protection to other Ants and attack other bees"""
    def __init__(self,health=2):
        super().__init__(health)
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if gamestate.is_queen_ant_exist:
            return None
        ret = super().construct(gamestate)
        gamestate.is_queen_ant_exist = True
        return ret
        # END Problem 12
