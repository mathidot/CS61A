...
def hefty_hogs(player_score, opponent_score):
    ...
    while opponent_score:
        ret, opponent_score = digit_fn(opponent_score % 10)(ret), opponent_score // 10
    return ret%30
    # END PROBLEM 2
...
def hog_pile(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    player_score_ones_digit,opponent_score_ones_digit = player_score % 10, opponent_score % 10
    if player_score_ones_digit == opponent_score_ones_digit:
        return player_score_ones_digit
    return 0
    # END PROBLEM 4
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if leader != None and leader != last_leader:
        return leader,f"Player {leader} takes the lead by {(score0 - score1) if leader == 0 else (score1 - score0)}"
    return leader,None
    # END PROBLEM 6
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    i = 1
    max_num_roll = 0
    highest_average_turn_score = -1
    while i <= 10:
        curren_average_turn_score = make_averaged(roll_dice,total_samples)(i,dice)
        if highest_average_turn_score < curren_average_turn_score:
            highest_average_turn_score = curren_average_turn_score
            max_num_roll = i
        i += 1
    return max_num_roll
    # END PROBLEM 9
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    temp_hefty_hogs = hefty_hogs(score,opponent_score)
    if (temp_hefty_hogs + score) % 10 == opponent_score % 10 \
        and opponent_score % 10 !=0:
        return 0
    if temp_hefty_hogs >= threshold:
        return 0
    return num_rolls
    # END PROBLEM 11
