...
(define (distance city-a city-b)
    (sqrt (+ (expt (- (get-lat city-a) (get-lat city-b)) 2) (expt (- (get-lon city-a) (get-lon city-b)) 2))))

(define (closer-city lat lon city-a city-b)
    (define temp (make-city 'temp lat lon))
    (if (< (distance city-a temp) (distance city-b temp)) 
        (get-name city-a)
        (get-name city-b)))
...
(define (student-attend-class student class)
    (student-create (get-name student) (cons class (student-get-classes student))))
