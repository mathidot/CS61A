...
class Account:
    ...
    def time_to_retire(self, amount):
        ...
        accumulate_balance = initial
        while accumulate_balance < amount:
            ret += 1
            accumulate_balance = accumulate_balance + accumulate_balance * self.interest
        return ret
...
class FreeChecking(Account):
    ...
    def withdraw(self, amount):
        ...
        if amount > self.max_withdrawal:
            self.withdraw_times +=  1
            return "Can't withdraw that amount"
        if self.withdraw_times < FreeChecking.free_withdrawals:
            self.withdraw_times += 1
            self.balance = self.balance - amount
        else:
            self.withdraw_times += 1
