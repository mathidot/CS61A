...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        function_bodys = expressions.rest
        procedure = do_lambda_form(Pair(function_formals,function_bodys),env)
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    expression_bodys = expressions.rest
    return LambdaProcedure(formals,expression_bodys,env)
    # END PROBLEM 7
...
def make_let_frame(bindings, env):
    ...
    while binding_pointer is not nil:
        name_val = binding_pointer.first
        validate_form(name_val,2,2)
        names = Pair(name_val.first,names)
        validate_formals(names)
        val = scheme_eval(name_val.rest.first,env)
        vals = Pair(val,vals)
